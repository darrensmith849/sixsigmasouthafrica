...
export default function DMAICFrameworkVisual({
  phase,
  hoverActive = false,
  pointerX = 0,
  pointerY = 0,
}: DMAICFrameworkVisualProps) {
  const stroke = "var(--muted2)";
  const accent = "var(--accent2)";

  const depth = hoverActive ? 6 : 0;
  const tx = pointerX * depth;
  const ty = pointerY * depth;
  const inverseTx = pointerX * -depth * 0.55;
  const inverseTy = pointerY * -depth * 0.55;
  ...
  const far = {
    transform: `translate(${inverseTx}px, ${inverseTy}px)`,
    transition: "transform 220ms cubic-bezier(0.22,1,0.36,1)",
  } as const;
  ...
  if (phase === "define") {
    return (
      <svg viewBox="0 0 260 160" fill="none" className="h-28 w-full" aria-hidden="true">
        ...
        <g style={near}>
          <circle cx="130" cy="80" r="34" stroke={accent} strokeWidth="1.5" strokeOpacity="0.7" />
          <circle cx="130" cy="80" r="17" stroke={accent} strokeWidth="1.2" strokeOpacity="0.5" />
    ...
    return (
      <svg viewBox="0 0 260 160" fill="none" className="h-28 w-full" aria-hidden="true">
        <g style={far}>
          ...
          <path d="M60 54 H202" stroke={stroke} strokeOpacity="0.3" strokeDasharray="3 5" />
        </g>
        <g style={near}>
          <rect x="62" y="92" width="18" height="28" rx="2" stroke={accent} strokeOpacity="0.55" />
          <rect x="96" y="76" width="18" height="44" rx="2" stroke={accent} strokeOpacity="0.65" />
          <rect x="130" y="66" width="18" height="54" rx="2" stroke={accent} strokeOpacity="0.75" />
          <rect x="164" y="82" width="18" height="38" rx="2" stroke={accent} strokeOpacity="0.55" />
        </g>
      </svg>
    );
  }
  ...
  if (phase === "analyse") {
    return (
      <svg viewBox="0 0 260 160" fill="none" className="h-28 w-full" aria-hidden="true">
        ...
        <g style={near}>
          <polyline points="44,112 78,92 110,101 146,70 182,82 216,48" stroke={accent} strokeWidth="1.5" strokeOpacity="0.75" />
          <circle cx="146" cy="70" r="20" stroke={accent} strokeOpacity="0.65" />
          <line x1="160" y1="84" x2="184" y2="108" stroke={accent} strokeOpacity="0.65" />
        </g>
      </svg>
    );
  }
  ...
  if (phase === "improve") {
    return (
      <svg viewBox="0 0 260 160" fill="none" className="h-28 w-full" aria-hidden="true">
        ...
        <g style={near}>
          <rect x="50" y="96" width="38" height="24" rx="4" stroke={accent} strokeOpacity="0.55" />
          <rect x="102" y="78" width="38" height="42" rx="4" stroke={accent} strokeOpacity="0.7" />
          <rect x="154" y="60" width="38" height="60" rx="4" stroke={accent} strokeOpacity="0.85" />
        </g>
      </svg>
    );
  }
  ...
  return (
    <svg viewBox="0 0 260 160" fill="none" className="h-28 w-full" aria-hidden="true">
      ...
      <g style={near}>
        <circle cx="86" cy="80" r="20" stroke={accent} strokeOpacity="0.58" />
        <circle cx="130" cy="80" r="20" stroke={accent} strokeOpacity="0.72" />
        <circle cx="174" cy="80" r="20" stroke={accent} strokeOpacity="0.58" />
