...
function PhaseVisual({ index }: { index: number }) {
  ...
  if (index === 0) {
    return (
      <svg viewBox="0 0 220 140" fill="none" className="h-24 w-full" aria-hidden="true">
        ...
        <circle cx="110" cy="70" r="30" stroke={accent} strokeOpacity="0.72" />
        <circle cx="110" cy="70" r="14" stroke={accent} strokeOpacity="0.55" />
      </svg>
    );
  }
  ...
  if (index === 1) {
    return (
      <svg viewBox="0 0 220 140" fill="none" className="h-24 w-full" aria-hidden="true">
        <rect x="14" y="14" width="192" height="112" rx="14" stroke={stroke} strokeOpacity="0.45" />
        <rect x="40" y="50" width="52" height="40" rx="6" stroke={accent} strokeOpacity="0.65" />
        <rect x="128" y="50" width="52" height="40" rx="6" stroke={accent} strokeOpacity="0.65" />
  ...
  if (index === 2) {
    return (
      <svg viewBox="0 0 220 140" fill="none" className="h-24 w-full" aria-hidden="true">
        <rect x="14" y="14" width="192" height="112" rx="14" stroke={stroke} strokeOpacity="0.45" />
        <polyline points="38,98 74,76 108,84 142,62 178,46" stroke={accent} strokeWidth="1.5" strokeOpacity="0.8" />
  ...
  if (index === 3) {
    return (
      <svg viewBox="0 0 220 140" fill="none" className="h-24 w-full" aria-hidden="true">
        <rect x="14" y="14" width="192" height="112" rx="14" stroke={stroke} strokeOpacity="0.45" />
        <circle cx="72" cy="70" r="16" stroke={accent} strokeOpacity="0.55" />
        <circle cx="110" cy="70" r="16" stroke={accent} strokeOpacity="0.72" />
        <circle cx="148" cy="70" r="16" stroke={accent} strokeOpacity="0.55" />
  ...
  return (
    <svg viewBox="0 0 220 140" fill="none" className="h-24 w-full" aria-hidden="true">
      ...
      <rect x="54" y="66" width="26" height="28" rx="4" stroke={accent} strokeOpacity="0.55" />
      <rect x="97" y="56" width="26" height="38" rx="4" stroke={accent} strokeOpacity="0.7" />
      <rect x="140" y="46" width="26" height="48" rx="4" stroke={accent} strokeOpacity="0.85" />
    </svg>
  );
}
...
export default function HowWeWorkStrip() {
  ...
  const active = phases[displayIndex];
      ...
      <div className="relative z-10 hidden md:block">
        ...
        <div className="grid grid-cols-12 gap-4 rounded-2xl border border-white/[0.08] bg-surface/55 px-5 py-4">
          ...
          <div className="col-span-5">
            <PhaseVisual index={displayIndex} />
          </div>
        </div>
      </div>
